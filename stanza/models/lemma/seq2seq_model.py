"""
The full encoder-decoder model, built on top of the base seq2seq modules that supports multiple predictions.
"""
...
class Seq2SeqModel(nn.Module):
    ...
    def predict(self, src, src_mask, pos=None, beam_size=5, n_preds=3):
        ...
        for b in range(batch_size):
            ...
            current_scores = []
            current_hyp = []
            for k in itertools.islice(ks, n_preds):
                hyp = beam[b].get_hyp(k)
                hyp = utils.prune_hyp(hyp)
                hyp = [i.item() for i in hyp]
                current_hyp.append(hyp)

            all_scores.append(scores)
            all_hyp.append(current_hyp)

        # TODO: how to deal with edit_logits
        return all_hyp, all_scores, edit_logits
