...
class POSProcessor(UDProcessor):
    ...
    def _set_up_model(self, config, use_gpu):
        ...
        if config.get('top_k_mode', 0) == 1:
            ...
            self._next_xpos = next_xpos
            self._next_best = NextBest
        elif config.get('top_k_mode', 0) == 3:
            # TODO: these have to be verified
            # Look at noun. Is it ok to have it a broad subset (all nouns, including proper)?
            # or a strict one?
            UPOS_MAPPINGS = {
            ...
            # Ignore XPOS score
            # TODO: figure out how to enable XPOS score
            self._scorer = lambda ups, xps, ufs: ups
            self._next_best = MultiNextBest
    ...
    def process(self, document):
        ...
        nb = self._next_best(preds, self._scorer, self._next_upos, self._next_xpos, self._next_ufeats)
    ...
    def __next__(self):
        ...
        for i in range(self._n_sents):
            ...
            for j in range(n_words):
                ...
                new_upos_index = upos_index_ret
                new_xpos_index = xpos_index_ret
                new_ufeats_index = ufeats_index_ret
